...
> **`azuread_access_package_catalog.description` is a REQUIRED provider argument.**
> Passing null fails with `The argument "description" is required`, which contradicts
> the promise that a catalog label needs no configuration at all. But the leaf module
> must reject a non-null description when `adopt_existing` is set, because the catalog
> belongs to another team. So the parent needs a generated default when creating and
> `null` when adopting — one cannot be a plain `coalesce`.
...
- `externally_visible` stays `false` by default, but be aware of what it costs.
  Entra evaluates catalog visibility **before** the assignment policy, so a catalog
  with `externally_visible = false` hides every package in it from guest users
  regardless of `requestor_scope_type` — and reports nothing anywhere. When the
  consumers are B2B guests, which is the normal case for consultants working in a
  customer tenant, both this and `requestor_scope_type` have to admit them.

> **`AllExistingDirectoryMemberUsers` excludes guests.** It is the provider's
> default and the obvious-looking choice, and it silently makes every package
> invisible to `userType == "Guest"`. `AllExistingDirectorySubjects` is the
> narrowest value covering members and guests already in the directory.
> `AllExternalSubjects` and the connected-organization scopes are broader and
> should not be reached for by accident.
>
> `SpecificDirectorySubjects` is a trap while the leaf module emits only
> `scope_type` and `requests_accepted` inside `requestor_settings`. With no
> `requestor` block the policy is scoped to specific subjects with none listed, so
> nobody can request — it passes validation and grants nothing. Either add a
